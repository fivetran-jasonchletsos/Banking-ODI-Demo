 ...
 *   BuildCompleteSummary — 4-pane summary replacing the one-line build-complete panel
 *   ModelRegistry        — small grouped view of the dbt project's existing models with the new one highlighted
 *   LiveBuildThumbnail   — auto-playing preview for the "Watch live build" CTA
 *
 * Zero-dep SVG and CSS. Aligned with the Pediment Bank navy/gold palette.
 */
...
// ─────────────────────────────────────────────────────────────────────────
// Vendor / accent tokens — keep consistent with OdiDbtWizardPage PILLARS
// ─────────────────────────────────────────────────────────────────────────
...
// 4 stages with gold particles flowing between them. Stages stack on mobile.
// ─────────────────────────────────────────────────────────────────────────
...
const PIPELINE_STAGES: Stage[] = [
  { key: 'src',  layer: 'Sources',        vendor: '8 bank systems',          stat: 'Core, cards, AML, watchlists', color: C.inkDim,  icon: 'S' },
  ...
  { key: 'dbt',  layer: 'Build-time AI',   vendor: 'dbt + dbt-wizard',        stat: '4 sub-agents · 90s/model',     color: C.dbt,      icon: 'W' },
    ...
    <div className="wiz-flow relative">
      ...
      <style>{`
        ...
        .wiz-flow .wiz-stage:hover {
          ...
          border-color: var(--accent, ${C.gold});
        ...
        .wiz-particle {
          ...
          background: linear-gradient(180deg, #fde68a 0%, ${C.gold} 100%);
          box-shadow: 0 0 8px ${C.gold}cc;
          left: 0;
        ...
        <div className="md:col-span-1 flex md:flex-col items-center justify-center" aria-hidden>
          <div className="hidden md:flex relative w-full h-7 items-center">
            <div
              ...
              style={{ background: `linear-gradient(90deg, ${C.gold}00, ${C.gold}77, ${C.gold}00)` }}
            />
            <span
          ...
          <div className="md:hidden flex items-center justify-center w-full py-1">
            <svg width="14" height="20" viewBox="0 0 14 20" fill="none" aria-hidden>
              <path d="M7 1 L7 17 M1 11 L7 17 L13 11" stroke={`${C.gold}aa`} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
      )}
...
const SPOKES: AgentSpoke[] = [
  { code: 'EX',  name: 'Explorer',     tools: 'status · search',     blurb: 'Maps what exists',     color: C.fivetran, angle: -135 },
  { code: 'SM',  name: 'Summary',      tools: 'describe · lineage',  blurb: 'Documents the schema', color: C.gold,     angle: -45  },
  { code: 'WK',  name: 'Worker',       tools: 'warehouse · dbt_show', blurb: 'Authors the SQL',     color: '#be185d',  angle:  45  },
...
// LineagePanel — live-evolving lineage graph for WizardLivePage.
//
// Driven by the build event step. Three upstream silver nodes appear in
// step 1, edges appear in step 2 as the join keys are validated, the new
// gold node materializes in step 4, gets a tests badge in step 5, and
// turns "live" in step 6.
// ─────────────────────────────────────────────────────────────────────────
...
export function LineagePanel({
  ...
  // Lay out the three upstream silvers on the left and the new gold on the right.
  const nodes: LineageNode[] = useMemo(() => {
    const upstream = scenario?.upstream_models ?? [];
    const silvers: LineageNode[] = upstream.slice(0, 3).map((u, i) => ({
      id: u.model,
  ...
  // Edges appear from step 2 onward — they represent the joins Summary validated.
...
function stepCaption(step: number, complete: boolean): string {
  ...
  if (step >= 1) return 'Explorer found 3 candidate silver tables';
  return 'Awaiting Explorer to map upstream candidates';
}

// ─────────────────────────────────────────────────────────────────────────
// BuildCompleteSummary — 4-pane summary replacing the one-line build-complete
// ─────────────────────────────────────────────────────────────────────────
...
export function BuildCompleteSummary({
  ...
  const panels: { title: string; stats: SummaryStat[]; accent: string }[] = [
    ...
    },
    {
      ...
      accent: C.iceberg,
      stats: [
        { label: 'Upstream refs', value: '3 silver' },
...
// ─────────────────────────────────────────────────────────────────────────
// ModelRegistry — small grouped view of the dbt project's existing models
// with the new one highlighted at the end.
// ─────────────────────────────────────────────────────────────────────────

type RegistryGroup = { layer: string; color: string; total: number; sample: string[]; };

const REGISTRY: RegistryGroup[] = [
  { layer: 'bronze', color: C.fivetran, total: 64, sample: ['fis_core_*', 'salesforce_fsc_*', 'plaid_aggregation_*', 'ncino_loan_*', 'visa_card_*'] },
  { layer: 'silver', color: '#94a3b8', total: 78, sample: ['stg_transactions', 'stg_merchants', 'stg_customers', 'stg_alerts', 'stg_branches'] },
  { layer: 'gold',   color: C.gold,    total: 26, sample: ['fct_fraud_alerts_daily', 'fct_aml_cases_weekly', 'dim_merchant', 'dim_customer', 'fct_loan_pipeline'] },
];
...
export function ModelRegistry({ newModelCode }: { newModelCode: string }) {
  ...
  return (
    <div className="research-card p-5">
      <div className="eyebrow mb-2">Model registry · 168 total</div>
      <h3 className="font-serif text-lg font-semibold mb-3" style={{ color: C.ink }}>
      ...
      <p className="mt-3 font-mono" style={{ fontSize: 11, color: 'var(--ink-soft)' }}>
        ...
        Downstream consumers — BI, AML, fraud desk — pick it up on next read.
      </p>
      <style>{`
        .registry-new {
...
// ─────────────────────────────────────────────────────────────────────────
// LiveBuildThumbnail — auto-playing 3-frame preview for the "Watch live
// build" CTA. Cycles between Explorer / Worker / Verification frames so the
// CTA feels like a real preview rather than a button.
// ─────────────────────────────────────────────────────────────────────────

const FRAMES: { agent: string; color: string; line1: string; line2: string }[] = [
  { agent: 'EXPLORER',     color: C.fivetran, line1: 'dbt search "cnp fraud"',          line2: 'found 3 silver candidates' },
  { agent: 'WORKER',       color: '#be185d',  line1: 'authoring fct_cnp_fraud_…sql',   line2: 'dbt_show ran on XS warehouse' },
  { agent: 'VERIFICATION', color: C.bull,     color2: '#0aa552', line1: 'dbt test --select +new',        line2: '7 tests passed · materialized' } as { agent: string; color: string; color2?: string; line1: string; line2: string },
];
