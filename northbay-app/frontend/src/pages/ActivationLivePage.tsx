/*
 * ActivationLivePage — NewCo Activations live-sync playback for Pediment Bank.
 *
 ...
 *
 * Vertical scenario: the BSA structured-deposit alert. The moment
 * gold.fct_structured_deposit_alerts flags an account with a composite
 * structuring_score above 0.85 tied to an ACH counterparty already linked to
 * a prior SAR filing, Activations pushes the case straight into NICE
 * Actimize Case Manager — no CSV export, no manual re-keying.
 */
...
const STEP_DEFS = [
  { label: 'Segment Definition',       who: 'Segment', tools: 'gold query',       insight: '21 accounts matched' },
  { label: 'Field Mapping',            who: 'Mapper',  tools: 'schema map',       insight: '5 fields mapped' },
  { label: 'Sync Preview',             who: 'Mapper',  tools: 'diff preview',     insight: '5 insert · 0 unchanged' },
  { label: 'API Push',                 who: 'Sync',    tools: 'REST push',       insight: '5 records sent' },
  { label: 'Destination Confirmation', who: 'Sync',    tools: 'destination read', insight: '5 landed · 0 errors' },
];

// ─── Vertical-specific scenario content (structured-deposit alert → Actimize) ─

const ACTIVATION_SCENARIO: ActivationScenario = {
  company: 'Pediment Bank',
  request_id: 'ACT-3391',
  requested_by: 'Structured Deposit Alert Monitor',
  requested_at: '2026-07-09T11:05:00Z',
  timezone_label: 'ET',
  question: 'Sync every flagged structuring account into Actimize before the 30-day SAR filing clock starts slipping.',
  source_model: 'gold.fct_structured_deposit_alerts',
  destination_system: 'NICE Actimize Case Manager',
  destination_object: 'Investigation_Case',
  sync_mode: 'insert',
  record_count: 5,
  build_room_seconds: 12,
};
...
const ACTIVATION_SCRIPT: ActivationEvent[] = [
  {
    ...
    body: "Watching gold.fct_structured_deposit_alerts for one condition: composite structuring_score crossing 0.85 on an ACH counterparty already tied to a prior SAR filing. 21 accounts just crossed the line, sharing 14 ACH counterparties — two of those counterparties already carry linked_to_prior_sar = true.",
    side_effect: 'gold.fct_structured_deposit_alerts · threshold crossed · 21 accounts',
    code_target: 'sql',
    code_append:
      "select\n  account_id,\n  structuring_score,\n  ach_counterparty_id,\n  branch_region,\n  prior_sar_reference,\n  linked_to_prior_sar\nfrom gold.fct_structured_deposit_alerts\nwhere structuring_score >= 0.85\n  and linked_to_prior_sar = true\norder by structuring_score desc;",
  },
  {
    from: 'mapper',
    step: 2,
    step_label: 'Field Mapping',
    body: "Field Mapping translates the governed gold columns straight into Actimize's case schema — account_id to Party_ID, structuring_score to Risk_Score, ach_counterparty_id to Related_Entity_ID, branch_region to Business_Unit, prior_sar_reference to Linked_SAR_Number. No custom API integration, no analyst keying a CSV by hand.",
    side_effect: 'mapping · 5 fields → Investigation_Case',
    code_target: 'json',
    code_append: JSON.stringify(
      {
        destination: 'NICE Actimize Case Manager',
        object: 'Investigation_Case',
        field_map: {
          account_id: 'Party_ID',
          structuring_score: 'Risk_Score',
          ach_counterparty_id: 'Related_Entity_ID',
          branch_region: 'Business_Unit',
          prior_sar_reference: 'Linked_SAR_Number',
        },
      },
      null,
      2,
  ...
  },
  {
    ...
    body: 'Sync Preview diffs against Actimize before anything pushes: 5 new investigation cases to open, 0 already-open cases affected. Every field lands typed and de-duplicated against the existing case load.',
    side_effect: 'diff · 5 insert · 0 unchanged',
    code_target: 'json',
    code_append: JSON.stringify({ to_insert: 5, unchanged: 0, to_update: 0 }, null, 2),
  },
  {
    from: 'sync',
    step: 4,
    step_label: 'API Push',
    body: "API Push sends the five case payloads straight into Actimize Case Manager's investigation queue — no CSV export out of Snowflake, no manual re-keying, no second source of truth.",
    side_effect: 'POST /v2/cases · NICE Actimize · 202 accepted',
    code_target: 'json',
    code_append: JSON.stringify(
      {
        object: 'Investigation_Case',
        Party_ID: 'ACC-77201',
        Risk_Score: 0.92,
        Related_Entity_ID: 'CPY-4410',
        Business_Unit: 'Northeast Retail',
        Linked_SAR_Number: 'SAR-2025-0447',
      },
      null,
      2,
    ),
  },
  {
    ...
    body: 'Destination Confirmation: 5 records synced to NICE Actimize — 12s ago. The BSA analyst opens a case to work, not a spreadsheet to transcribe, and the 30-day SAR filing clock starts against a case that is already pre-populated, not four to six hours later.',
    side_effect: 'NICE Actimize · 5 cases created · 0 errors',
  },
];
...
const ACTIVATION_RECORDS: ActivationRecord[] = [
  {
    ...
    fields: {
      'Party ID': 'ACC-77201',
      'Risk Score': 0.92,
      'Related Entity ID': 'CPY-4410',
      'Business Unit': 'Northeast Retail',
      'Linked SAR Number': 'SAR-2025-0447',
    },
    status: 'created',
  },
  {
  ...
  },
  {
    ...
    fields: {
      'Party ID': 'ACC-77142',
      'Risk Score': 0.91,
      'Related Entity ID': 'CPY-2286',
      'Business Unit': 'Mid-Atlantic Retail',
      'Linked SAR Number': 'SAR-2024-1183',
    },
    status: 'created',
  },
  {
...
export default function ActivationLivePage() {
  ...
  const codeLabel =
    currentEvent?.code_target === 'sql'
      ? 'models/gold/fct_structured_deposit_alerts.sql'
  ...
  return (
    <div className="wizard-terminal mx-auto max-w-[1640px] px-4 py-4 sm:px-6 lg:px-8">
      ...
      <div className="mb-3 px-4 py-2.5 research-card border-l-4 flex items-center gap-5 flex-wrap" style={{ borderLeftColor: '#0e7490' }}>
        <div className="min-w-0 flex-shrink" style={{ flex: '1 1 460px' }}>
          <div className="eyebrow" style={{ fontSize: 10, marginBottom: 2, color: '#0e7490' }}>
            BSA / AML · {scenario.timezone_label} · {scenario.requested_by}
          </div>
      ...
      <div className="mb-3 grid gap-1.5" style={{ gridTemplateColumns: 'repeat(5, minmax(0, 1fr))' }}>
        {STEP_DEFS.map((s, idx) => {
          ...
          const accentColor = active ? '#0e7490' : done ? 'var(--bull)' : 'var(--hairline)';
          return (
            <div
              key={s.label}
              ...
              style={{
                ...
                background: active ? 'rgba(14,116,144,0.06)' : done ? 'var(--bull-bg)' : 'var(--paper-deep)',
              }}
              title={`${s.who} · ${s.tools}`}
            >
              <div
                className="font-mono font-bold flex items-center gap-1.5"
                style={{ fontSize: 10, letterSpacing: '0.04em', color: active ? '#0e7490' : done ? 'var(--bull)' : 'var(--ink-soft)' }}
              >
              ...
              <div
                ...
                style={{ fontSize: 10, lineHeight: 1.25, color: active ? '#0e7490' : done ? 'var(--bull)' : 'var(--ink-soft)', opacity: done || active ? 0.95 : 0.55 }}
                title={s.insight}
              >
                {s.insight}
              </div>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1.25fr)' }}>
        ...
        <section className="research-card flex flex-col min-h-[60vh] lg:min-h-[300px] lg:h-[calc(100dvh-440px)]">
          <header className="px-5 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--hairline)' }}>
            <div>
        ...
        <section className="flex flex-col gap-3 min-h-[60vh] lg:min-h-[300px] lg:h-[calc(100dvh-440px)]" style={{ minWidth: 0 }}>
          <div className="research-card flex flex-col" style={{ flex: '1 1 0', minHeight: 0 }}>
            <header className="px-5 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--hairline)' }}>
      ...
      {complete && (
        <div className="mt-6 research-card p-5" style={{ borderLeft: '5px solid var(--bull)', background: 'var(--bull-bg)' }}>
          <div className="flex items-baseline justify-between flex-wrap gap-3 mb-1">
            <div className="flex items-baseline gap-3 flex-wrap">
              <div
                className="status-pill bull shrink-0"
                style={{ display: 'inline-flex', fontSize: 12, padding: '4px 10px', fontWeight: 700 }}
              >
                Sync Complete
              </div>
      ...
      <style>{`
        ...
        .wizard-terminal::after {
          content: 'pediment-bank/activations-live · NewCo Activations';
          position: absolute;
          top: 0; left: 0; right: 0;
          height: 28px;
          display: flex;
        ...
        .wizard-terminal .research-card[style*="var(--bull-bg)"] {
